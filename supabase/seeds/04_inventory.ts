...
export const seedInventoryAndCollections = async (
  ...
  for (const library of createdLibraries) {
    ...
    for (const edition of selectedEditions) {
      ...
      for (let i = 0; i < copyCount; i++) {
        ...
        const currentBorrower = availabilityStatus === 'borrowed' ? 
          faker.helpers.arrayElement(libraryMembers)?.id || null : null;
        ...
        const result = await seed.book_copies([{
          ...
          condition_info: {
            ...
            last_maintenance: condition === 'good' ? null : faker.date.recent({ days: 180 }).toISOString(),
            notes: condition === 'damaged' ? 'Minor wear on cover' : null,
          },
          availability: {
            status: availabilityStatus,
            current_borrower_id: currentBorrower,
            due_date: availabilityStatus === 'borrowed' ? faker.date.future({ days: 14 }).toISOString() : null,
            hold_queue: [],
          },
          status: 'active',
        }]);
    ...
    await supabase
      ...
      .update({ 
        stats: {
          ...
          active_loans: createdBookCopies.filter(c => c.library_id === library.id && c.availability.status === 'borrowed').length,
  ...
  for (const library of createdLibraries) {
    ...
    for (const [index, collectionData] of collections.entries()) {
      ...
      const libraryBooks = createdBookCopies.filter(c => c.library_id === library.id);
      const collectionBooks = faker.helpers.arrayElements(libraryBooks, faker.number.int({ min: 5, max: 15 }));
