...
test.describe("i18n Language Switching E2E", () => {
  ...
  test("should switch from English to Vietnamese and persist preference", async ({ page }) => {
    ...
    const languageSwitcher = page.locator('[data-testid="language-switcher"], button:has-text("English"), button:has(svg):has-text("English")').first();
    ...
    await expect(page.locator('text="Bảng điều khiển", text="Quản lý thư viện"').first()).toBeVisible({ timeout: 5000 });
    
    // Verify the language switcher now shows Vietnamese
    const updatedSwitcher = page.locator('button:has-text("Tiếng Việt")').first();
  ...
  test("should maintain language preference across page navigation", async ({ page }) => {
    ...
    const navigationMenu = page.locator('nav, .sidebar, [role="navigation"]').first();
    if (await navigationMenu.isVisible()) {
      // Try to find and click a navigation link
      const settingsLink = page.locator('text="Cài đặt", text="Settings", a:has-text("Settings")').first();
    ...
    const persistedSwitcher = page.locator('button:has-text("Tiếng Việt")').first();
  ...
  test("should maintain language preference after browser refresh", async ({ page }) => {
    ...
    await expect(page.locator('text="Bảng điều khiển", text="Quản lý thư viện"').first()).toBeVisible();
  });
  ...
  test("should switch back from Vietnamese to English", async ({ page }) => {
    ...
    let vietnameseOption = page.locator('text="Tiếng Việt"').first();
    await vietnameseOption.click();
