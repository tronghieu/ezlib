...
test.describe("Book Management", () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.goto("/test-library/books");
  ...
  test.describe("Books List Page", () => {
    test("should display books page with correct layout", async ({ page }) => {
      ...
      await expect(page.locator("p")).toContainText("Manage your library's book inventory");
      ...
      await expect(page.locator("th")).toContainText(["Title", "Author", "Publisher", "Year", "ISBN", "Status"]);
    });
  ...
  test.describe("Sorting", () => {
    test("should sort by title", async ({ page }) => {
      ...
      const titles = await page.locator("table tbody td:first-child").allTextContents();
    ...
    test("should sort by author", async ({ page }) => {
      ...
      const authors = await page.locator("table tbody td:nth-child(2)").allTextContents();
    ...
    test("should toggle sort direction", async ({ page }) => {
      ...
      const firstTitleAsc = await page.locator("table tbody td:first-child").first().textContent();
      
      // Second click - descending  
      await titleButton.click();
      await page.waitForTimeout(300);
      
      const firstTitleDesc = await page.locator("table tbody td:first-child").first().textContent();
  ...
  test.describe("Pagination", () => {
    ...
    test("should navigate between pages", async ({ page }) => {
      ...
      if (await nextButton.isEnabled()) {
        ...
        const firstBookPage1 = await page.locator("table tbody td:first-child").first().textContent();
        ...
        const firstBookPage2 = await page.locator("table tbody td:first-child").first().textContent();
  ...
  test.describe("Navigation", () => {
    ...
    test("should maintain library context in URLs", async ({ page }) => {
      ...
      expect(page.url()).toContain("/test-library/books");
      
      // Navigation should preserve library context
      await page.locator('text="Add New Book"').click();
      expect(page.url()).toContain("/test-library/books/add");
    });
  });
  ...
  test.describe("Accessibility", () => {
    test("should be keyboard navigable", async ({ page }) => {
      ...
      await expect(page.locator('input[placeholder*="Search books"]')).toBeFocused();
