...
test.describe("Cross-Domain Session Sharing Tests", () => {
  ...
  test("1.3-CDS-001: Independent session management for manage.ezlib.com", async ({ page }) => {
  ...
  test("1.3-CDS-002: Session persistence across page navigation", async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      sessionStorage.setItem("library-context", JSON.stringify({
  ...
  test("1.3-CDS-004: Cross-domain architecture preparation", async ({ page, context }) => {
  ...
  test("1.3-CDS-005: Session state synchronization with auth changes", async ({ page }) => {
  ...
  test("1.3-CDS-006: Future cross-domain session sharing architecture", async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      localStorage.setItem("supabase.auth.token", JSON.stringify(crossDomainSession));
    });
  ...
  test("1.3-CDS-007: Session domain context validation", async ({ page }) => {
    ...
    expect(domainContext.domain_roles["manage.ezlib.com"]).toBe("library-admin");
  });

  test("1.3-CDS-008: Session refresh across domain context", async ({ page }) => {
...
test.describe("Cross-Domain Session Security Tests", () => {
  test("1.3-CDS-SEC-001: Session isolation between domains", async ({ page, context }) => {
  ...
  test("1.3-CDS-SEC-003: Cross-domain session sharing security", async ({ page }) => {
    ...
    const sharingData = await page.evaluate(() => {
      ...
      const hasSensitiveData = JSON.stringify(sharableData).includes("sensitive_info");
