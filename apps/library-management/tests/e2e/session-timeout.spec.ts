...
test.describe("Session Timeout Scenario Tests", () => {
  ...
  test("1.3-STO-003: Automatic token refresh before expiry", async ({ page }) => {
    ...
    await page.evaluate(() => {
      const currentToken = JSON.parse(localStorage.getItem("supabase.auth.token")!);
    ...
    expect(refreshedToken.expires_at).toBeGreaterThan(Date.now() + (50 * 60 * 1000)); // At least 50 minutes remaining
  });

  test("1.3-STO-004: Failed token refresh redirects to login", async ({ page }) => {
    ...
    await page.evaluate(() => {
      localStorage.setItem("supabase.auth.token", JSON.stringify({
        ...
        expires_at: Date.now() + (5 * 60 * 1000), // Expires in 5 minutes
        user: { id: "failing-refresh-user", email: "failingrefresh@example.com" }
      }));
    });
    ...
    await expect(page.getByText("Session could not be refreshed")).toBeVisible();
  });
  ...
  test("1.3-STO-005: Idle session timeout", async ({ page }) => {
    ...
    await expect(page.getByRole("button", { name: "Stay Logged In" })).toBeVisible();
  ...
  test("1.3-STO-007: Session timeout during form submission", async ({ page }) => {
    ...
    await expect(page.getByText("Your session expired while submitting")).toBeVisible();
  ...
  test("1.3-STO-008: Multiple tab session synchronization", async ({ page, context }) => {
    ...
    await page.evaluate(() => {
      const expiredToken = JSON.parse(localStorage.getItem("supabase.auth.token")!);
  ...
  test("1.3-STO-009: Session recovery after network reconnection", async ({ page }) => {
  ...
  test("1.3-STO-010: Graceful session cleanup on browser close", async ({ page, context }) => {
    // Given: User with active session
    await page.evaluate(() => {
      localStorage.setItem("supabase.auth.token", JSON.stringify({
...
test.describe("Session Timeout Edge Cases", () => {
  test("1.3-STO-EDGE-001: System clock changes during session", async ({ page }) => {
    ...
    await page.evaluate((time) => {
      ...
      if (clockSkew > 5 * 60 * 1000) { // More than 5 minutes skew
        const adjustedToken = {
          ...token,
          clock_skew_detected: true,
          server_time_offset: serverTime - clientTime
        };
        localStorage.setItem("supabase.auth.token", JSON.stringify(adjustedToken));
      }
    }, realTime);
  ...
  test("1.3-STO-EDGE-002: Concurrent session refresh attempts", async ({ page }) => {
