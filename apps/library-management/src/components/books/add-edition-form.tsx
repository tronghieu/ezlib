...
import type { BookEdition, Author, BookEditionFormData } from "@/lib/types/books";
...
const editionFormSchema = z.object({
  ...
  publication_year: z
    ...
    .optional(),
  publisher: z.string().max(255, "Publisher name must be less than 255 characters").optional(),
  isbn: z.string()
  ...
  const onSubmit = async (data: EditionFormData): Promise<void> => {
    ...
    createBookEdition.mutate(editionData, {
      ...
      onError: (err) => {
        ...
        setError(err instanceof Error ? err.message : "Failed to create book edition");
      },
    });
  };
    ...
    <>
      <div className="space-y-6">
        <div className="space-y-2">
          <h3 className="text-lg font-medium">Add New Book Edition</h3>
          <p className="text-sm text-muted-foreground">
            First, select an author or create a new one, then fill in the book details.
          </p>
        </div>
          ...
          <CardContent>
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              ...
              <div className="flex flex-col sm:flex-row gap-3 pt-6 border-t">
                ...
                >
                  {createBookEdition.isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
                  Create Book Edition
                </Button>
              ...
              <div className="text-xs text-muted-foreground bg-muted/50 p-3 rounded-md">
                <strong>Note:</strong> All fields except Title and Language are optional. 
                You can add more details later. ISBN should be 10 or 13 digits.
              </div>
            </form>
          </CardContent>
        </Card>
