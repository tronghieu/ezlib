...
describe('Book Validation Schema', () => {
  ...
  describe('addBookSchema validation', () => {
    ...
    test('2.2-UNIT-029: should validate title character limits', () => {
      ...
      if (!result.success) {
        const titleError = result.error.issues.find(e => e.path[0] === 'title');
        expect(titleError?.message).toBe('Title must be less than 255 characters');
      }
    });
    ...
    test('2.2-UNIT-030: should validate author character limits', () => {
      ...
      if (!result.success) {
        const authorError = result.error.issues.find(e => e.path[0] === 'author');
        expect(authorError?.message).toBe('Author name must be less than 255 characters');
      }
    });
    ...
    test('2.2-UNIT-031: should validate publication year range', () => {
      ...
      if (!result.success) {
        const yearError = result.error.issues.find(e => e.path[0] === 'publication_year');
    ...
    test('2.2-UNIT-032: should validate ISBN format', () => {
      ...
      if (!result.success) {
        ...
        expect(isbnError?.message).toBe('Invalid ISBN format - must be a valid 10 or 13 digit ISBN');
      }
    });
    ...
    test('2.2-UNIT-038: should validate publisher character limits', () => {
      ...
      if (!result.success) {
        const publisherError = result.error.issues.find(e => e.path[0] === 'publisher');
        expect(publisherError?.message).toBe('Publisher name must be less than 255 characters');
      }
    });
    ...
    test('2.2-UNIT-039: should only contain specified fields', () => {
      ...
      const expectedFields = ['title', 'author', 'publisher', 'publication_year', 'isbn'];
      
      expect(Object.keys(schemaShape)).toEqual(expectedFields);
      
      // Ensure no complex cataloging fields are present
      const complexFields = ['genre', 'subjects', 'dewey_decimal', 'lccn', 'description', 'cover_url'];
