...
async function isCrawlerServiceAvailable(): Promise<boolean> {
  try {
    ...
    const crawlerUrl = process.env.NEXT_PUBLIC_CRAWLER_SERVICE_URL || 'http://localhost:8000';
 ...
 */
export async function enrichFromISBN(isbn: string): Promise<BookMetadata | null> {
  if (!validateISBN(isbn)) {
    return null;
  }
  ...
  try {
    ...
    if (!response.ok) {
      console.warn('ISBN enrichment failed:', response.status, response.statusText);
      return null;
    }
