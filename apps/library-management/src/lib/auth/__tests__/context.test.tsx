...
import { AuthProvider, useAuthContext, withAuth, withLibraryAccess } from '../context';
...
function TestComponent() {
  ...
  return (
    <div>
      ...
      <div data-testid="library-access">{auth.hasLibraryAccess ? 'has-access' : 'no-access'}</div>
...
const AuthenticatedTestComponent = withAuth(() => <div data-testid="protected">Protected Content</div>);
const LibraryAccessTestComponent = withLibraryAccess(() => <div data-testid="library-content">Library Content</div>);
...
describe('AuthContext Tests', () => {
  ...
  beforeEach(() => {
    ...
    mockSessionManager.refreshSession.mockResolvedValue({ session: null, error: null });
  });
  ...
  describe('AuthProvider initialization', () => {
    ...
    it('should load user session on initialization', async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByTestId('user-email')).toHaveTextContent('test@library.com');
      });
  ...
  describe('Authentication state management', () => {
    ...
    it('should handle auth state changes', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId('user-email')).toHaveTextContent('new@library.com');
        expect(screen.getByTestId('authenticated')).toHaveTextContent('authenticated');
      });
    });
  ...
  describe('Library context management', () => {
    it('should handle library access state', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId('library-access')).toHaveTextContent('has-access');
      });
    });
  ...
  describe('Higher-Order Components', () => {
    ...
    it('should show library content for withLibraryAccess when has access', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId('library-content')).toHaveTextContent('Library Content');
      });
    });
  });
