...
describe('Permission System Tests', () => {
  ...
  describe('hasPermission function', () => {
    ...
    it('should handle custom permissions', () => {
      ...
      expect(hasPermission(userWithCustomPermissions, 'reports:export')).toBe(true);
    ...
    it('should handle denied permissions', () => {
      ...
      expect(hasPermission(userWithDeniedPermissions, 'books:delete')).toBe(false);
      expect(hasPermission(managerUser, 'books:delete')).toBe(true);
    });
    ...
    it('should prioritize denied permissions over granted ones', () => {
      ...
      expect(hasPermission(userWithConflictingPermissions, 'system:backup')).toBe(false);
    });
  });

  describe('hasAnyPermission function', () => {
    it('should return true if user has any of the specified permissions', () => {
      expect(hasAnyPermission(librarianUser, ['books:view', 'system:admin'])).toBe(true);
      expect(hasAnyPermission(librarianUser, ['system:admin', 'staff:manage'])).toBe(false);
    });

    it('should return false if user has none of the specified permissions', () => {
      expect(hasAnyPermission(librarianUser, ['system:admin', 'books:delete'])).toBe(false);
    });
  });

  describe('hasAllPermissions function', () => {
    it('should return true if user has all specified permissions', () => {
      expect(hasAllPermissions(librarianUser, ['books:view', 'members:view'])).toBe(true);
      expect(hasAllPermissions(ownerUser, ['books:delete', 'system:admin'])).toBe(true);
    });

    it('should return false if user is missing any specified permission', () => {
      expect(hasAllPermissions(librarianUser, ['books:view', 'books:delete'])).toBe(false);
    });
  });
  ...
  describe('getUserPermissions function', () => {
    ...
    it('should not include duplicate permissions', () => {
      ...
      const bookViewCount = permissions.filter(p => p === 'books:view').length;
  ...
  describe('requirePermission function', () => {
    it('should not throw for valid permissions', () => {
      expect(() => requirePermission(librarianUser, 'books:view')).not.toThrow();
      expect(() => requirePermission(ownerUser, 'system:admin')).not.toThrow();
    });

    it('should throw PermissionError for invalid permissions', () => {
      expect(() => requirePermission(librarianUser, 'system:admin')).toThrow(PermissionError);
      expect(() => requirePermission(managerUser, 'system:admin')).toThrow(PermissionError);
    });
  ...
  describe('Permission descriptions and categories', () => {
    it('should provide descriptions for all permissions', () => {
      const allPermissions: Permission[] = [
        'books:view', 'books:add', 'books:edit', 'books:delete', 'books:bulk',
        'members:view', 'members:add', 'members:edit', 'members:delete', 'members:bulk',
        'circulation:checkout', 'circulation:return', 'circulation:renew', 'circulation:holds',
        'reports:view', 'reports:export', 'reports:advanced',
        'settings:view', 'settings:edit', 'settings:policies',
        'staff:view', 'staff:invite', 'staff:manage', 'staff:permissions',
        'system:admin', 'system:backup', 'system:audit'
      ];
    ...
    it('should organize permissions into categories', () => {
      ...
      expect(PERMISSION_CATEGORIES.members.permissions).toContain('members:add');
      expect(PERMISSION_CATEGORIES.circulation.permissions).toContain('circulation:checkout');
      expect(PERMISSION_CATEGORIES.system.permissions).toContain('system:admin');
    });
  ...
  describe('Edge cases and error handling', () => {
    it('should handle undefined custom permissions', () => {
      ...
      expect(() => hasPermission(userWithoutCustom, 'books:view')).not.toThrow();
